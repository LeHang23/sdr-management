...
function renderSourceDescription(source) {
  if (source.mode === 'simulator') {
    fleetSourceTitle.textContent = 'Remote simulator';
    fleetSourceCopy.textContent = 'Virtual SDR devices are reporting through the authenticated gateway.';
    return;
  }
  if (source.mode === 'sdr_gateway') {
    fleetSourceTitle.textContent = 'SDR Gateway';
    fleetSourceCopy.textContent = 'Physical SDR telemetry is connected through the gateway.';
    return;
  }
  fleetSourceTitle.textContent = 'Manual data source';
  fleetSourceCopy.textContent = 'SQLite demo data. Update it manually to simulate fleet changes.';
}

export function renderFleetSummary(summary, { stale = false } = {}) {
...
export function renderFleetSummaryUnavailable(message) {
